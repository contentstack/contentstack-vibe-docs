...
**Read [SKILL.md](SKILL.md) first.** It contains the routing table that maps your current task to the 1–3 reference files you need. The full skill spans ~13,500 lines across 32 reference documents — do not read them all.

## How to use this repo

1. Open [SKILL.md](SKILL.md) and match the user's task to the routing table.
2. Read only the referenced file(s) from `references/`.
...
Never ask for, log, or hardcode API keys or tokens. Use `process.env.*`. Management Tokens are server-side only — never expose them in frontend code. See the **Red Flags** section in `SKILL.md`.
